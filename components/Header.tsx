  ...
  return (
    <header className="bg-gray-800 shadow-md p-4 print-hide">
      <div className="max-w-[1920px] mx-auto flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-cyan-400">طراح شبکه نوری پسیو (POL)</h1>
          <p className="text-gray-400">معماری شبکه POL خود را به صورت سرتاسری مصورسازی و پیکربندی کنید.</p>
        </div>
        <div className="flex items-center space-x-4">
          <span className="text-gray-400">خوش آمدید، <span className="font-semibold text-cyan-300">{user.email}</span></span>
          {canAccessAdmin && (
            <button
            ...
            >
              {isAdminOpen ? 'بستن پنل ادمین' : 'پنل ادمین'}
            </button>
          )}
          <button
            onClick={onLogout}
