      ...
      <div className="flex-grow flex items-center justify-center min-h-[400px]">
        <div className="w-full flex items-center flex-row-reverse">
          ...
          <div className="flex flex-col items-center gap-2 pl-4 text-center">
              ...
              <p className="text-xs text-gray-500">شبکه Core</p>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-xs text-gray-400">{parameters.ponPorts} پورت ({parameters.sfpSelection})</p>
          </div>

          {/* Backbone Fiber */}
          <div className="flex-1 h-2 bg-gradient-to-l from-cyan-500 to-cyan-700 mx-2 rounded-full relative">
            <span className="absolute -top-6 left-1/2 -translate-x-1/2 bg-gray-700 px-2 py-0.5 rounded text-xs whitespace-nowrap">{parameters.backboneDistance}متر Backbone</span>
          </div>

          {/* Splitter(s) */}
          {splitterConfig.type === 'Centralized' ? (
              renderSplitter(splitterConfig.level1Ratio, 'اسپلیتر')
          ) : (
              <>
                {renderSplitter(splitterConfig.level1Ratio, 'اسپلیتر سطح ۱')}
                <div className="flex-1 h-0.5 bg-yellow-400 mx-2"></div>
                {renderSplitter(splitterConfig.level2Ratio, 'اسپلیتر سطح ۲')}
              </>
          )}

          {/* Splitter to ONT paths */}
          <div className="flex-1 flex flex-col justify-center items-start pr-2 relative">
             <div className="w-full h-0.5 bg-yellow-400 absolute right-0 top-1/2 -translate-y-1/2"></div>
             <div className="w-0.5 h-full bg-yellow-400 absolute right-2 top-0"></div>

             {Array.from({ length: ontsToDisplay }).map((_, i) => (
                 <div key={i} className={`h-full w-full relative ${i > 0 ? 'mt-2':''}`}>
                     <div style={{width: `${(i + 1) * (100 / (ontsToDisplay+1))}%`}} className="h-0.5 bg-yellow-400 absolute top-1/2 -translate-y-1/2 left-0"></div>
                 </div>
             ))}
          </div>
          ...
          <div className="flex flex-col space-y-3 pr-2">
            {Array.from({ length: ontsToDisplay }).map((_, index) => (
              <div key={index} className="flex items-center">
                ...
                <OntIcon className="w-8 h-8 text-green-400 mr-2" />
              </div>
            ))}
             {parameters.ontsPerPonPort > ontsToDisplay && (
                <div className="text-center text-xs text-gray-500">
                    ... و {parameters.ontsPerPonPort - ontsToDisplay} دستگاه دیگر
                </div>
             )}
          </div>
        </div>
       ...
       {powerMargin < 0 && (
        <div className="mt-4 p-3 bg-red-900/50 border border-red-500 rounded-lg text-center">
          <p className="font-bold text-red-400">هشدار: حاشیه توان منفی</p>
          <p className="text-sm text-red-300">این طراحی قابل اجرا نیست. سیگنال برای رسیدن به ONT بسیار ضعیف است. تلفات را کاهش دهید یا ماژول SFP قوی‌تری انتخاب کنید.</p>
        </div>
      )}
    </div>
  );
