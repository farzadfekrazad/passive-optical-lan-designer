...
const VisualizationPanel: React.FC<VisualizationPanelProps> = ({ parameters, selectedOlt }) => {
  ...
  const ontsToDisplay = Math.min(parameters.ontsPerPonPort, 8);
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg shadow-lg h-full flex flex-col">
      <h2 className="text-xl font-bold mb-4 text-cyan-400 border-b border-gray-700 pb-2">{t('visualization.title')}</h2>
      
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-gray-900/50 p-4 rounded-md">
        <div className="text-center">
          <p className="text-sm text-gray-400">{t('visualization.oltTxPower')}</p>
          <p className="text-lg font-semibold text-cyan-300">{parameters.oltTxPower.toFixed(1)} dBm</p>
        </div>
        <div className="text-center">
          <p className="text-sm text-gray-400">{t('visualization.totalLoss')}</p>
          <p className="text-lg font-semibold text-red-400">{totalLoss.toFixed(2)} dB</p>
        </div>
         <div className="text-center">
          <p className="text-sm text-gray-400">{t('visualization.estimatedOntPower')}</p>
          <p className="text-lg font-semibold text-cyan-300">{receivedPower.toFixed(2)} dBm</p>
        </div>
        <div className="text-center">
          <p className="text-sm text-gray-400">{t('visualization.powerMargin')}</p>
          <p className={`text-2xl font-bold ${getMarginColor()}`}>{powerMargin.toFixed(2)} dB</p>
        </div>
      </div>
      
      <div className="flex-grow flex items-center justify-center min-h-[400px]">
        <div className="w-full flex items-center flex-row rtl:flex-row-reverse">
          {/* Uplink/Core */}
          <div className="flex flex-col items-center gap-2 text-center ltr:pr-4 rtl:pl-4">
              <ServerIcon className="w-12 h-12 text-gray-400" />
              <p className="text-xs text-gray-500">{t('visualization.coreNetwork')}</p>
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-xs text-gray-400">{t('visualization.ponPortsCount', {count: parameters.ponPorts, sfp: parameters.sfpSelection})}</p>
          </div>

          {/* Backbone Fiber */}
          <div className="flex-1 h-2 bg-gradient-to-l rtl:bg-gradient-to-r from-cyan-500 to-cyan-700 mx-2 rounded-full relative">
            <span className="absolute -top-6 left-1/2 -translate-x-1/2 bg-gray-700 px-2 py-0.5 rounded text-xs whitespace-nowrap">{t('visualization.backboneDistance', {distance: parameters.backboneDistance})}</span>
          </div>

          {/* Splitter(s) */}
          {splitterConfig.type === 'Centralized' ? (
              renderSplitter(splitterConfig.level1Ratio, t('visualization.splitter'))
          ) : (
              <div className="flex items-center">
                {renderSplitter(splitterConfig.level1Ratio, t('visualization.level1Splitter'))}
                <div className="flex-1 h-0.5 bg-yellow-400 mx-2 min-w-[2rem]"></div>
                {renderSplitter(splitterConfig.level2Ratio, t('visualization.level2Splitter'))}
              </div>
          )}

          {/* Splitter to ONT paths */}
          <div className="flex-1 flex flex-col justify-center items-start rtl:items-end ltr:pr-2 rtl:pl-2 relative">
             <div className="w-full h-0.5 bg-yellow-400 absolute ltr:right-0 rtl:left-0 top-1/2 -translate-y-1/2"></div>
             <div className="w-0.5 h-full bg-yellow-400 absolute ltr:right-2 rtl:left-2 top-0"></div>

             {Array.from({ length: ontsToDisplay }).map((_, i) => (
                 <div key={i} className={`h-full w-full relative ${i > 0 ? 'mt-2':''}`}>
                     <div style={{width: `${(i + 1) * (100 / (ontsToDisplay+1))}%`}} className="h-0.5 bg-yellow-400 absolute top-1/2 -translate-y-1/2 ltr:left-0 rtl:right-0"></div>
                 </div>
             ))}
          </div>

          {/* ONTs */}
          <div className="flex flex-col space-y-3 ltr:pr-2 rtl:pl-2">
            {Array.from({ length: ontsToDisplay }).map((_, index) => (
              <div key={index} className="flex items-center">
                <div className="flex-1 h-0.5 bg-yellow-400"></div>
                <OntIcon className="w-8 h-8 text-green-400 ltr:ml-2 rtl:mr-2" />
              </div>
            ))}
             {parameters.ontsPerPonPort > ontsToDisplay && (
                <div className="text-center text-xs text-gray-500">
                    {t('visualization.andMore', { count: parameters.ontsPerPonPort - ontsToDisplay })}
                </div>
             )}
          </div>
        </div>
       ...
       {powerMargin < 0 && (
        <div className="mt-4 p-3 bg-red-900/50 border border-red-500 rounded-lg text-center">
          <p className="font-bold text-red-400">{t('visualization.warningTitle')}</p>
          <p className="text-sm text-red-300">{t('visualization.warningText')}</p>
        </div>
      )}
    </div>
  );
